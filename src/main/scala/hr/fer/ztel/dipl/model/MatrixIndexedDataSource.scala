...
object MatrixIndexedDataSource extends Serializable {

    def createItemItemMatrix(path : String, measure : ItemPairSimiliarityMeasure,
      partitioner : Option[Partitioner] = None, useIndexing : Boolean = false)
      (implicit spark : SparkSession) : (RDD[(Int, Map[Int, Double])], Map[Int, Int]) = {
      ...
      val idIndexMap : Map[Int, Int] =
        (customerItemRecords.map(_._1).distinct union customerItemRecords.map(_._2).distinct)
          .distinct.rdd.zipWithIndex.mapValues(_.toInt).collectAsMap.toMap

      val customerItemRDD = customerItemRecords
        .flatMap {
      ...
      if (partitioner.isDefined)
        (customerItemRDD
          ...
          .mapValues(itemVector => itemVector.toMap), idIndexMap)
